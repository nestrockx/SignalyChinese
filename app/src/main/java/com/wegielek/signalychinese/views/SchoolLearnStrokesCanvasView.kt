...
class SchoolLearnStrokesCanvasView : View {
    ...
    private var schoolWritingViewModel: SchoolWritingViewModel? = null
    ...
    fun setViewModel(schoolWritingViewModel: SchoolWritingViewModel) {
        this.schoolWritingViewModel = schoolWritingViewModel
        schoolWritingViewModel.characterMode.observe((context as SchoolWritingActivity)) {
            if (it === CharacterMode.PRESENTATION) {
                valueStrokeAnimator.duration = 500
                schoolWritingViewModel.setLastMode(it)
    ...
    fun setHanziCharacter(hanziChar: Char, isChanged: Boolean) {
        ...
        if (isChanged) {
            schoolWritingViewModel?.getLastMode()?.let { schoolWritingViewModel?.setCharacterMode(it) }
            return
        }
        clear()
        valueStrokeAnimator.cancel()
        hanziCharacter = initHanziStrokes(hanziChar)
        if (schoolWritingViewModel?.getCharacterMode() == CharacterMode.PRESENTATION) {
    ...
    private fun initHanziStrokes(hanziChar: Char): HanziCharacter {
        ...
        try {
            ...
            if (schoolWritingViewModel?.getCharacterMode() != CharacterMode.NOT_FOUND) {
                schoolWritingViewModel?.getCharacterMode()
                    ?.let { schoolWritingViewModel?.setLastMode(it) }
            }
            schoolWritingViewModel?.setCharacterMode(CharacterMode.NOT_FOUND)
            schoolWritingViewModel?.isContinue?.value = !schoolWritingViewModel?.isContinue?.value!!
    ...
    private fun drawHanziCharacterPath() {
        if (schoolWritingViewModel?.getCharacterMode() === CharacterMode.LEARN) {
    ...
    private fun touchStart(x: Float, y: Float) {
        if (schoolWritingViewModel?.getCharacterMode() !== CharacterMode.PRESENTATION) {
            ...
            mX = x
            mY = y
            if (hanziCharacter!!.matchStart(mX, mY, 70.0f)) {
    ...
    private fun touchUp() {
        if (schoolWritingViewModel?.getCharacterMode() !== CharacterMode.PRESENTATION) {
            if (hanziCharacter!!.isMatchedStart && hanziCharacter!!.isMatchedControlPoint) {
                if (hanziCharacter!!.matchEnd(mX, mY, 70.0f)) {
